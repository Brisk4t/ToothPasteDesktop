...
use super::{auth_label, firmware_label, CurrentScreen, ToothPasteTUI};
...
fn render_device_list(frame: &mut Frame, app: &mut ToothPasteTUI, area: Rect) {
    ...
    let header = Paragraph::new(
        Line::from(vec![
            Span::styled("Discovered devices", Style::default().add_modifier(Modifier::BOLD)),
...
fn render_connected(frame: &mut Frame, app: &ToothPasteTUI, area: Rect) {
    ...
    let text = Text::from(vec![
        Line::from(vec![
            ...
            Span::styled(&dev.name, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::raw("  Address:   "),
        ...
        Line::from(""),
        Line::from(
            Span::styled("  <D> Disconnect", Style::default().fg(Color::DarkGray))
        ),
    ]);
...
fn render_pairing(frame: &mut Frame, app: &ToothPasteTUI, area: Rect) {
    ...
    let input_text = Paragraph::new(app.pair_input.as_str())
...
fn render_side_panel(frame: &mut Frame, app: &ToothPasteTUI, area: Rect) {
    ...
    match &state.connected_device {
        Some(dev) => {
            ...
            let auth_color = if auth == "Authenticated" { Color::Green }
                else if auth == "Auth failed" { Color::Red }
                else { Color::Yellow };
        ...
        None => {
            lines.push(Line::from(
                Span::styled("○ No device connected", Style::default().fg(Color::DarkGray))
            ));
        }
    }
