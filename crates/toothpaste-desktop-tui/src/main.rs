...
async fn main() -> io::Result<()> {
    ...
    let app_state_rx = spawn_ipc_bridge(stream, cmd_rx);

    tokio::task::block_in_place(|| tui::start_tui(app_state_rx, cmd_tx))
}
...
fn spawn_ipc_bridge(stream: Stream, cmd_rx: mpsc::Receiver<AppCommand>) -> watch::Receiver<AppState> {
    let (app_state_tx, app_state_rx) = watch::channel(AppState::default());
    tokio::spawn(ipc_bridge(stream, app_state_tx, cmd_rx));
    app_state_rx
}
