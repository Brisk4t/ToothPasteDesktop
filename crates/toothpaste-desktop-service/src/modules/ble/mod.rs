...
impl BleManager {
    ...
    pub async fn ble_connect_toothpaste(
        ...
        let service = self
            .get_service_with_retry(&peripheral, SERVICE_UUID)
            .await?;
        let packet_char = self
            .get_characteristic_with_retry(&service, PACKET_CHARACTERISTIC_UUID)
            .await?;
        let semaphore_char = self
            .get_characteristic_with_retry(&service, HID_SEMAPHORE_CHARACTERISTIC_UUID)
            .await?;
        let mac_char = self
            .get_characteristic_with_retry(&service, MAC_ADDRESS_CHARACTERISTIC_UUID)
            .await?;

        let mac_data = self
            .connected_peripherial
            .as_ref()
            .unwrap()
            .read(&mac_char)
            .await?;
    ...
    async fn get_service_with_retry(
        &self, peripheral: &Peripheral, uuid: Uuid,
    ) -> Result<btleplug::api::Service, Box<dyn std::error::Error>> {
        let attempts = 3;
    ...
    async fn get_characteristic_with_retry(
        &self, service: &btleplug::api::Service, uuid: Uuid,
    ) -> Result<btleplug::api::Characteristic, Box<dyn std::error::Error>> {
        let attempts = 3;
        for attempt in 0..attempts {
            match service
                .characteristics
                ...
                .cloned()
            {
                ...
                None => {
                    if attempt < attempts - 1 {
                        eprintln!(
                            "Retrying characteristic discovery... (attempt {})",
                            attempt + 1
                        );
                        time::sleep(Duration::from_millis(300)).await;
                    } else {
    ...
    pub async fn ble_send_unencrypted_packet(
        &self, data: &[u8],
        ...
        self.connected_peripherial
            .as_ref()
            .unwrap()
            .write(packet_char, data, btleplug::api::WriteType::WithoutResponse)
            .await?;
        Ok(())
    }
